...
async function attachDebugger(tabId): Promise {
...
async function handleToolRequest(request): Promise {
  const { method, params } = request;

  try {
    switch (method) {
      case 'tabs_list': {
        const tabs = await chrome.tabs.query(params | undefined);
        return {
          success: true,
          result: tabs.map((tab) => ({
            id: tab.id!,
      ...
      case 'tabs_update': {
        ...
        Object.keys(updateProperties).forEach(key => 
          (updateProperties)[key] === undefined && delete (updateProperties)[key]
        );
      ...
      case 'console_read': {
        const { pattern, limit } = (params as { pattern?; limit? }) || {};
      ...
      case 'cookies_get': {
        const { url, name } = params as { url; name? };
      ...
      case 'resize_window': {
        const { tabId, width, height } = params as { tabId; width; height };
